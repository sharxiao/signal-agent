...
from src.agent.config import RAW_ARTICLES_FILE, CHROMA_COLLECTION_NAME
...
from src.agent.chunker import build_chunks, save_chunks
...
def get_retriever():
    """
    Return a cached (collection, embedding_client) tuple for app-time retrieval.

    IMPORTANT:
    - During ingestion/build, we use get_or_create_collection(...)
    - During app-time retrieval, we should only open the existing collection
      to avoid write/create operations on Streamlit Cloud's SQLite-backed Chroma
    """
    embedding_client = get_embedding_client()
    client = get_chroma_client()
